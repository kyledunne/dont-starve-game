...
public class Color extends Style {
	...
	public void draw(Rect rect) {
		float[] xCoords = rect.getDrawingBoundaryXCoords();
		float[] yCoords = rect.getDrawingBoundaryYCoords();
		if (xCoords.length != yCoords.length)
			throw new RuntimeException("xCoords length != yCoords length (which is silly!)");
		
		if(a != 0) {
			Color.glColor4f(this);
	...
	@Override
	public Color getHighlightedStyle() {
		Color c = new Color(this);
		if(a <= .7f)
			c.a += .3f;
		else
			if(isCloserToBlack(c))
				c = Color.adjustAColor(c, .3f, .3f, .3f);
			else
				c = Color.adjustAColor(c, -.3f, -.3f, -.3f);
		return c;
	}
	...
	public Style getSelectedStyle() {
		Color c = new Color(getHighlightedStyle());
		if(c.a <= .7f) {
			c.a += .3f;
		}
		else
